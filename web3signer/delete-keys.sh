...
for client in "${CLIENTS_TO_REMOVE[@]}"; do
  case "$client" in
  "prysm")
    CLIENT_API="http://validator.prysm-ropsten.dappnode:3500"
    TOKEN_FILE="/security/prysm/auth-token"
    CERT_REQUEST=""
    ;;
  "teku")
    CLIENT_API="https://validator.teku-ropsten.dappnode:3500"
    ...
    ;;
  "lighthouse")
    CLIENT_API="http://validator.lighthouse-ropsten.dappnode:3500"
    TOKEN_FILE="/security/lighthouse/api-token.txt"
    CERT_REQUEST=""
    ;;
  "nimbus")
    CLIENT_API="http://beacon-validator.nimbus-ropsten.dappnode:3500"
